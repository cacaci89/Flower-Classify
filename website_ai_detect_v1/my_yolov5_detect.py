...
def yolo_detect_face(img_origin, model_yolo, model_gender, model_age, img_size = 416):
    ...
    for obj_id, (*xyxy, conf, class_num) in enumerate(det_scaled):
        ...
        ### 若沒有進行分類任務 以下程式碼可以刪除----------------------
        # 進行年齡與性別的辨識 圖檔轉成年齡模型需要的大小與維度-->face_img
        # 前處理 模型輸入為 (1,224,224,3) RGB 
        face_img = cv2.resize(obj_img, (224,224)) # 我訓練的原始的模型是224x224
        face_img = np.expand_dims(face_img,axis=0)

        face_img = preprocess_input(face_img) # InceptionV3的前處理

        # 預測年齡  辨認性別
        age = model_age.predict(face_img)[0][0]
        age = int(age) # 年齡取整數即可
        
        gender_preded = model_gender.predict(face_img)
        # 性別的編號
        gender_num = np.argmax( gender_preded[0] )
        gender_proba = float(gender_preded[0][gender_num]) #轉成python float
        gender_proba = int(gender_proba*100) # 取整數 顯示百分比
        #gender_proba = round(gender_proba, 2) # 取小數點下兩位
        ...
        obj_info["gender"] = {"gender": gender_label, "proba": gender_proba}
        # print(obj_info)
        ...
        box_label = "{},{}{}%age{}".format(obj_id, gender_label, gender_proba, age) #加上年齡與性別文字資訊
        # draw box on img_result
